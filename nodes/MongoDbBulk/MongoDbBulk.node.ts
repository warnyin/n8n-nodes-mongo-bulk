	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				options: [
					...
					},
					{
						...
						type: 'json',
						default: '[]',
						description: 'Array of filter documents for array update operations',
					},
				],
			},
			...
			// Bulk Write Options
			{
				...
				placeholder: '[{"insertOne": {"document": {"name": "John"}}}, {"updateOne": {"filter": {"_id": "123"}, "update": {"$set": {"status": "active"}}}}]',
			},
			{
				displayName: 'Options',
				name: 'options',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		if (credentials.configurationType === 'connectionString') {
			...
			const auth = credentials.user
				? `${credentials.user}:${encodeURIComponent(credentials.password as string)}@`
				: '';
				...
				try {
					...
					switch (operation) {
						case 'insertMany': {
							...
							const documents = typeof documentsJson === 'string'
						...
						case 'updateMany': {
							...
							const update = typeof updateJson === 'string'
								? JSON.parse(updateJson)
								: updateJson;
							const options = this.getNodeParameter('options', i, {}) as any;

							// Parse array filters if provided
							if (options.arrayFilters && typeof options.arrayFilters === 'string') {
							...
							result = await mongoCollection.updateMany(filter, update, options);
							break;
						}
						...
						case 'find': {
							...
							if (options.sort) {
								const sort = typeof options.sort === 'string'
									? JSON.parse(options.sort)
									: options.sort;
								cursor = cursor.sort(sort);
							}
							if (options.projection) {
								const projection = typeof options.projection === 'string'
						...
						case 'bulkWrite': {
							...
							const operations = typeof operationsJson === 'string'
							...
							const processedOperations = operations.map((op: any) => {
								...
								if (operation.insertOne?.document?._id && typeof operation.insertOne.document._id === 'string') {
									operation.insertOne.document._id = new ObjectId(operation.insertOne.document._id);
								}
								if (operation.updateOne?.filter?._id && typeof operation.updateOne.filter._id === 'string') {
									operation.updateOne.filter._id = new ObjectId(operation.updateOne.filter._id);
								}
								if (operation.updateMany?.filter?._id && typeof operation.updateMany.filter._id === 'string') {
									operation.updateMany.filter._id = new ObjectId(operation.updateMany.filter._id);
								}
								if (operation.deleteOne?.filter?._id && typeof operation.deleteOne.filter._id === 'string') {
									operation.deleteOne.filter._id = new ObjectId(operation.deleteOne.filter._id);
								}
								if (operation.deleteMany?.filter?._id && typeof operation.deleteMany.filter._id === 'string') {
									operation.deleteMany.filter._id = new ObjectId(operation.deleteMany.filter._id);
								}
								if (operation.replaceOne?.filter?._id && typeof operation.replaceOne.filter._id === 'string') {
									operation.replaceOne.filter._id = new ObjectId(operation.replaceOne.filter._id);
								}

								return operation;
							});

							result = await mongoCollection.bulkWrite(processedOperations, options);
							break;
						}
